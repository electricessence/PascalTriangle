...
namespace Sharith.Factorial
{
	...
	public class ParallelSplitPrimeSwing : IFactorialFunction
	{
		...
		private delegate BigInteger SwingDelegate(PrimeSieve sieve, int n);
		...
		private BigInteger RecFactorial2(int n)
		{
			if (n < 2) return BigInteger.One;

			var recFact = RecFactorial2(n / 2);
			var sqrFact = BigInteger.Pow(recFact, 2);
			var swing = n < Smallswing
					  ? SmallOddSwing2[n]
					  : swingDelegate2.EndInvoke(results2[--taskCounter2]);

			return sqrFact * swing;
		}

		private BigInteger RecFactorial3(int n)
		{
			if (n < 2) return BigInteger.One;

			var recFact = RecFactorial3(n / 2);
			var sqrFact = BigInteger.Pow(recFact, 2);
			var swing = n < Smallswing
					  ? SmallOddSwing3[n]
					  : swingDelegate3.EndInvoke(results3[--taskCounter3]);

			return sqrFact * swing;
		}

		private static BigInteger Swing2(PrimeSieve sieve, int n)
		{
		...
		private static BigInteger Swing3(PrimeSieve sieve, int n)
		{
			var primorial = Task.Factory.StartNew(() =>
			{
				var start = sieve.NextPrime(n / 2);
		...
		static readonly BigInteger[] SmallOddSwing2 = {
		...
		static readonly BigInteger[] SmallOddSwing3 = {
