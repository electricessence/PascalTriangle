...
namespace Sharith.Factorial
{
	...
	public class SplitPrimeSwing : IFactorialFunction
	{
		...
		private BigInteger OddSwing(uint n)
		{
			if (n < SmallOddSwing.Length)
			{
				return new BigInteger(SmallOddSwing[n]);
			}
		...
		public BigInteger Factorial(int N)
		{
			...
			{
				return new BigInteger(SmallOddFactorial[n]) << exp2;
			}
		...
		private BigInteger OddFactorial(uint n)
		{
			return n < SmallOddFactorial.Length ? new BigInteger(SmallOddFactorial[n]) : BigInteger.Pow(OddFactorial(n / 2), 2) * OddSwing(n);
		}
  ...
  147926426347074375, 3698160658676859375 };
	...
	class Primes
	{
		...
		public BigInteger GetPrimorial(uint min, uint max)
		{
	...
	static public class MathFun
	{
		...
		static public BigInteger Product(long[] seq, int start, int len)
		{
			...
			{
				var halfLen = len / 2;
				var rprod = BigInteger.Zero;
				var lprod = BigInteger.Zero;
